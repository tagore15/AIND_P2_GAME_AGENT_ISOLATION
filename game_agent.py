...
#    return float(2*own_moves - opp_moves)
    return float(2*own_moves - opp_moves)/(len(game.get_blank_spaces())+1)
...
class CustomPlayer:
    ...
    def minimax(self, game, depth, maximizing_player=True):
        ...
        for move in legal_moves:
            ...
            score, _ = self.minimax(newGame, depth-1, not maximizing_player)
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf"), maximizing_player=True):
        ...
        for move in legal_moves:
            newGame = game.forecast_move(move)
            score, _ = self.alphabeta(newGame, depth-1, alpha, beta, not maximizing_player)
            if maximizing_player == True:
                if score >= beta:
                    bestScore = score 
                    break
